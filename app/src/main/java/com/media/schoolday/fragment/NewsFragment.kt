...
class NewsFragment: Fragment(), AnkoLogger {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        ibNewsPostChild.setOnClickListener {
            val sekolah = spinner.selectedItem.toString()
            callback?.onUserClick(sekolah)
//            context.toast(sekolah)
...
//        spinner.onItemSelectedListener
    ...
    fun addPhoto(photo: String,option: String){
        ...
        try {
            ...
            viewPhoto.tag = index
            viewPhoto.ibImagetDelete.setOnClickListener {
                linear.removeView(viewPhoto.findViewWithTag(index))
                listPhoto.remove(index to photo)
            }
